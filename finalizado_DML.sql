...
-- SELECT product_id, UPPER(product_name) AS product_name, unit_price FROM products WHERE LENGTH(product_name) = 10;
...
-- SELECT customer_id, company_name, city, country FROM customers ORDER BY city, country ASC;
...
-- SELECT ship_city, SUM(freight) AS suma_envio FROM orders WHERE ship_city = 'Madrid' OR ship_city = 'Sevilla' OR ship_city = 'Barcelona' OR ship_city = 'Lisboa' GROUP BY ship_city ORDER BY suma_envio;
...
-- SELECT category_id, COUNT(product_id) AS cantidad_de_productos FROM products WHERE unit_price >= 10 AND unit_price <=60 AND units_in_stock > 12 GROUP BY category_id ORDER BY category_id;
...
-- SELECT category_id, SUM(units_in_stock) AS unidades_existencia FROM products WHERE supplier_id = 17 OR supplier_id = 19 OR supplier_id = 16 GROUP BY category_id ORDER BY category_id;

--Cuya categoria tenga menos de 100 unidades ordenado por unidades
-- SELECT category_id, SUM(units_in_stock) AS unidades_existencia FROM products GROUP BY category_id HAVING SUM(units_in_stock) <= 100 ORDER BY unidades_existencia;
